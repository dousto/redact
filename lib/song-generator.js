...
var SongGenerator = function(config) {
  config = config || {};
  this.rng = config.rng;
  this.tempo = config.tempo || 80;
  this.key = config.key || 0;
  this.keyQuality = config.keyQuality || 'major';
  this.chordsInstrument = config.chordsInstrument || 0;
  this.chordsVelocityAdjust = config.chordsVelocityAdjust || 0;
  this.melodyInstrument = config.melodyInstrument || 0;
  this.melodyVelocityAdjust = config.melodyVelocityAdjust || 0;
};

SongGenerator.prototype.generate = function(number, callback) {
  var config = {rng: this.rng, key: 'key(' + this.key + ', ' + this.keyQuality + ')'};
...
SongGenerator.prototype.convertToMidi = function(song, callback) {
  ...
  file.addTrack([new File.MetaEvent(File.MetaEvent.TYPE.SET_TEMPO, {
    ...
    program: this.chordsInstrument
  }, 0)])
    .addTrack([new File.MetaEvent(File.MetaEvent.TYPE.SET_TEMPO, {
      tempo: this.tempo
    }), new File.ChannelEvent(File.ChannelEvent.TYPE.PROGRAM_CHANGE, {
      program: this.melodyInstrument
    }, 1)]);
  ...
  var chordEvents = asToMidi.convertToMidiEvents(song.chordNotes, {
    scale: 4*128,
    channel: 0,
    velocityAdjust: this.chordsVelocityAdjust
  });
  chordEvents.forEach(function(event) {
    chordTrack.addEvent(event)
  });
  debug('Build melody midi events');
  var melodyEvents = asToMidi.convertToMidiEvents(song.melodyNotes, {
    scale: 64,
    channel: 1,
    velocityAdjust: this.melodyVelocityAdjust
  });
